      ...
      {isEditing ? (
        <div className="edit-box">
          <div className="edit-title">
            <input
              ...
              type="text"
              value={editedTitle}
              aria-label="Edit the task title"
              onChange={(e) => setEditedTitle(e.target.value)}
            />
            <button className="fake-button my-button" aria-label="Fake edit button">
          ...
          <input
            ...
            aria-label="Edit the task decription"
            onChange={(e) => setEditedDescription(e.target.value)}
          />
          <div className="edit-buttons-box">
            <div className="edit-save-or-cancel">
              <button className="edit-save my-text-button" onClick={handleSaveEditClick} aria-label="Save the input edit">Save</button>
              <button className="edit-cancel my-text-button" onClick={handleCancelEditClick} aria-label="Cancel the input edit">Cancel</button>
            </div>
            <button className="fake-button my-button" aria-label="Fake delete button">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" width="16" height="16" strokeWidth={1} stroke="currentColor" className="size-6">
        ...
        <div>
          <div className="title-box">
            ...
            <Tooltip id={`my-edit-${task.id}`} className="custom-tooltip" border="1px solid var(--tooltip-border)" />
          </div>
          ...
          <div className="task-footer">
            ...
            <Tooltip id={`my-delete-${task.id}`} className="custom-tooltip" border="1px solid var(--tooltip-border)" />
            <div className={`confirmation-box ${isConfirmVisible ? 'show' : ''}`}>
              <button className="confirm-delete my-text-button" onClick={handleConfirmDelete} aria-label="Confirm deleting the task">Delete</button>
              <button className="confirm-cancel my-text-button" onClick={handleCancel} aria-label="Cencel deleting the task">Cancel</button>
            </div>
          </div>
        </div>
      )}
